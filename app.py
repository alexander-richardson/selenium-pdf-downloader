...
chrome_driver_path = os.path.join(os.getcwd(), "chromedriver.exe")  # Make sure chromedriver mathces your Chrome version
...
for pdf_url in pdf_urls:
    ...
    time.sleep(5)  # Wait for 5 seconds to ensure the download completes

# Before logout, clear browser cache for all time
print("Clearing browser cache for all time...")
driver.get('chrome://settings/clearBrowserData')  # Navigate to Chrome's Clear Browsing Data page
time.sleep(2)
...
actions.send_keys(Keys.TAB * 3)  # Move to the 'Time range' dropdown (assuming it's at the right position)
actions.send_keys(Keys.DOWN)  # Select 'All time' (you may need to adjust if it's different)
